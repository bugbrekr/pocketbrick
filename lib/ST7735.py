...
class TFT(object) :
  ...
  def __init__( self, spi, aDC, aReset, aCS, rotation=0) :
    """aLoc SPI pin location is either 1 for 'X' or 2 for 'Y'.
       aDC is the DC pin and aReset is the reset pin."""
    self._size = ScreenSize
    self._offset = bytearray([0,0])
    self.rotate = rotation                    #Vertical with top toward pins.
...
#   @micropython.native
  def vline( self, aStart, aLen, aColor ) :
    ...
    aStart = self._fix_coords(aStart)
...
#   @micropython.native
  ...
  def fill( self, aColor = BLACK ) :
    ...
    self.fillrect((0, 0), (128, 128), aColor)

  def image( self, x0, y0, x1, y1, data ) :
    x0, y0 = self._fix_coords((x0, y0))
    x1, y1 = self._fix_coords((x1, y1))
